...
public static class ParchisBoardFactory
{
    ...
    private static void AddFinishLaneConnections(List<(string, string)> connections, string color, string entryTrackPosition)
    {
        ...
        for (int i = 1; i < ParchisBoard.FinishLaneSize - 1; i++)
        {
            ...
            connections.Add(($"{color}_finish_{i}", ParchisBoard.Center));
        }

        // Connect last finish lane to center
        connections.Add(($"{color}_finish_{ParchisBoard.FinishLaneSize - 1}",ParchisBoard.Center));
    }

    private static void AddSpawnAreas(List<(string, string)> connections)
    {
        connections.Add((ParchisBoard.RedSpawn, ParchisBoard.RedEntry));
        connections.Add((ParchisBoard.BlueSpawn, ParchisBoard.BlueEntry));
        connections.Add((ParchisBoard.GreenSpawn, ParchisBoard.GreenEntry));
        connections.Add((ParchisBoard.YellowSpawn, ParchisBoard.YellowEntry));
