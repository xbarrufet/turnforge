...
/// </summary>
public sealed class TurnOrderState(
    ImmutableList<PlayerId> playerOrder,
    int currentPlayerIndex,
    int roundNumber
)
{
    public ImmutableList<PlayerId> PlayerOrder { get; } = playerOrder;
    ...
    /// </summary>
    public TurnOrderState NextPlayer() => new(PlayerOrder, CurrentPlayerIndex + 1, RoundNumber);
    
    
    /// <summary>
    /// Start a new round (reset to first player, increment round).
    /// </summary>
    public TurnOrderState NextRound() => new(PlayerOrder, 0, RoundNumber+1);
